...
read.top <- function(file) {
  ...
  parse <-function(con){
    ...
    col_classes <- c("integer","character","character","character","character","character","character","character","numeric","numeric","character","character","character")
    ...
    read.threads <- function(nthreads,snapshot) {
      t <- as.data.frame(scan(file=con, nlines = nthreads, what=sapply(col_classes,do.call,list(0)), skip = 4, strip.white = T,fill = T,
                              na.strings = "",quiet = T), stringsAsFactors=F) 
      names(t) <- col_names
  ...
  tryCatch(parse(con), finally = close(con))
}